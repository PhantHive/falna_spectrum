...
| `FFT()` | `np.fft.fft()` | IDL divides by N automatically whereas Python is returning the raw fft result                                                                                                                   |
| `ABS()` | `np.abs()` |                                                                                                                                                                                                 |
| `MAX()` | `np.max()` |                                                                                                                                                                                                 |
| `MEAN()` | `np.mean()` |                                                                                                                                                                                                 |
| `MRDFITS()` | `astropy.io.fits.open()` | in MRDFITS we can specify which element to read by specifying an index: `MRDFITS('..', 1)` whereas in python we have first to `fits.open('..')` then explicitly access the table `data[1].data` |
...
| Path string      | Paths can be written using `\`               | Paths must be written using `\\`  or use escape character `r`                   |
